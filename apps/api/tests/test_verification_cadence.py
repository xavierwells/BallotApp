...
from app.verification_cadence import BUILT_IN_CADENCE, VerificationCadence, resolve_cadence
...
def test_builtin_cadence_matches_the_approved_stage_policy() -> None:
    ...
    assert BUILT_IN_CADENCE.interval_for(
        today=today, next_election_date=None, official_ballot_available=False
    ) == timedelta(days=30)
    assert BUILT_IN_CADENCE.interval_for(
        today=today, next_election_date=date(2026, 11, 3), official_ballot_available=False
    ) == timedelta(days=7)
    assert BUILT_IN_CADENCE.interval_for(
        today=today, next_election_date=date(2026, 11, 3), official_ballot_available=True
    ) == timedelta(days=1)
