...
- [ ] Import official county, precinct, municipal, school-district, legislative, and special-district boundaries. TLC's 2026-primary precinct ZIP is now pinned with exact source metadata, schema, license, checksum, and expected Bell/Coryell/Lampasas counts in a repeatable draft/reference manifest. Running that import and obtaining local-authority confirmation for November use remain open; municipal, school, legislative, and special-district sources remain separate work.
...
- [ ] For unresolved addresses, return and display the evidence-backed plausible ballot set with geographic explanations, source citations, and no preselected winner. The API pipeline returns this evidence-backed set; web presentation remains.
